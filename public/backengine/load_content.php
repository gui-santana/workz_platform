<?php 
$moderador = 0;	
//PROFILE
if(isset($_GET['profile']) && !isset($_GET['company']) && !isset($_GET['team']) && !isset($_GET['post']) && !isset($_GET['p'])){	
	if(!empty($_GET['profile'])){
		$js_page_type = 'profile';
		$user = search('hnw', 'hus', '', "id = '".$_GET['profile']."'");		
		$get_count = count($user);		
		if($get_count > 0){
			$user = $user[0];
			//VERIFICA PUBLICAÇÕES			
			$post_count = count(search('hnw','hpl','id',"us = '".$_GET['profile']."' AND em = '' AND cm = '' AND st = '1'"));		
			//FOLLOWERS
			$followers = array();					
			foreach(search('hnw', 'usg', 's0', "s1 = '".$_GET['profile']."'") as $res){
				foreach($res as $key => $data){
					if($key == 's0'){
						$followers[] = $data;
					}
				}
			}
			//FOLLOWED
			$followed = array();
			foreach(search('hnw', 'usg', 's1', "s0 = '".$_GET['profile']."'") as $res){
				foreach($res as $key => $data){
					if($key == 's1'){
						$followed[] = $data;
					}
				}
			}
											
			//USER'S COMPANIES
			$companies = array_unique(array_column(search('cmp', 'employees', 'em', "us = '".$_GET['profile']."' AND st > 0 AND nv > 0"), 'em'));
			$blocked_companies = array_unique(array_column(search('cmp', 'companies', 'id', "id IN (".implode(',', $companies).") AND pg = 0"), 'id'));	
			$companies = array_diff($companies, $blocked_companies);
			
			//USER'S TEAMS			
			$teams = search('cmp', 'teams_users', 'cm,st', "us = '".$_GET['profile']."'");				
			foreach($teams as $key => $team){				
				$cm = $team['cm'];
				$st = $team['st'];
				$team = search('cmp', 'teams', 'pg,em', "id = '".$cm."'");
				if($cm == 0 || $team[0]['pg'] == 0 || !in_array($team[0]['em'], $companies) || $st == 0){
					unset($teams[$key]);
				}
			}
			$teams = array_values(array_unique(array_column($teams, 'cm')));								
			
			//USER'S DATA		
			$pgft = 'us';
			$pgid = $user['id'];			
			$pgtt = $user['tt'];
			$pgun = $user['un'];			
			$pgpc = $user['pg'];
			$ctpc = $user['pc'];
			$pgim = $user['im'];
			if(empty($user['im'])){ $pgim =  ''; }			
			//NEW
			$pgbk = $user['bk'];
			$pgcl = $user['cl'];
			$pgds = $user['cf'];			
			//USER LEVEL
			$user_level = '';
			if((isset($_SESSION['wz']) && ($_GET['profile'] == $_SESSION['wz'])) || (isset($_SESSION['wz']) && in_array($_SESSION['wz'], $followers)) || $ctpc == 3){
				$user_level = 1;
			}
		}
	}
//TEAM
}elseif(isset($_GET['team']) && !isset($_GET['profile']) && !isset($_GET['company']) && !isset($_GET['post']) && !isset($_GET['p'])){
	
	if(!empty($_GET['team'])){
		$js_page_type = 'team';		
		$team = search('cmp', 'teams', '', "id = '".$_GET['team']."'");
		$get_count = count($team);
		if($get_count > 0){
			$team = $team[0];
			//TEAM'S USERS
			$users = array();
			$users_level = array();					
			foreach(search('cmp', 'teams_users', 'us,st', "st > 0 AND cm = '".$_GET['team']."'") as $key => $data){			
				$users[] = $data['us'];
				$employee_data = search('cmp', 'employees', 'nv', "st > 0 AND em = ".$team['em']." AND us = ".$data['us']);				
				if (!empty($employee_data) && $employee_data[0]['nv'] > 0) {
					$users_level[] = $employee_data[0]['nv'];
				} else {							
					unset($users[$key]);
				}				
			}			
			//TEAM'S POSTS			
			$team_posts = search('hnw', 'hpl', 'id', "cm = '".$_GET['team']."'");
			//TEAM'S DATA
			$pgft = 'cm';
			$pgid = $team['id'];
			$pgpc = $team['pg'];
			$ctpc = $team['pc'];
			$pgtt = $team['tt'];
			$pgun = $team['un'];
			$pgim = $team['im'];
			$pgus = $team['us'];
			$pgtp = $team['tp'];
			$pgem = $team['em'];
			$pgdt = $team['dt'];
			//NEW
			$pgbk = $team['bk'];
			$pgcl = $team['cl'];
			if($pgem <> ''){				
				$company = search('cmp', 'companies', 'ex,bk,cl', "id = '".$pgem."'");
				if(count($company) > 0){
					$company = $company[0];
					if($company['ex'] == 1){
						$pgbk = $company['bk'];
						$pgcl = $company['cl'];
					}
				}
			}
			$pgds = $team['cf'];					
			//MODERATORS
			$moderators = array();							
			foreach(search('cmp', 'employees', 'us', "st > 0 AND nv > 1 AND em = '".$team['em']."'") as $res){
				foreach($res as $key => $data){
					if($key == 'us'){
						if(search('cmp', 'teams_users', 'id', "us = '".$data."' AND cm = '".$_GET['team']."' AND st > 0")){
							$moderators[] = $data;
						}						
					}
				}
			}
			//ACCESS REQUESTS
			$requests = array();			
			foreach(search('cmp', 'teams_users', 'us', "st = 0 AND cm = '".$_GET['team']."'") as $res){
				foreach($res as $key => $data){
					if($key == 'us'){
						$requests[] = $data;
					}
				}		
			}
			//USER LEVEL
			$user_level = '';

			if(isset($_SESSION['wz'])){

				// Verifica se o usuário tem vínculo ativo com a empresa				
				if ($company_access = search('cmp', 'employees', 'id,nv', "st > 0 AND em = '".$team['em']."' AND us = '".$_SESSION['wz']."'")) {
					// Agora verifica o vínculo com a equipe
					$team_user = search('cmp', 'teams_users', 'st', "cm = '{$_GET['team']}' AND us = '{$_SESSION['wz']}'");									

					if (!empty($team_user)) {
						if ($team_user[0]['st'] == 0) {
							$user_level = 0; // Solicitação pendente
						} elseif ($team_user[0]['st'] == 1) {
							$user_level = $company_access[0]['nv']; // Membro da equipe
						}
					} else {
						$user_level = ''; // Não faz parte da equipe ainda, mas pode solicitar
					}

				} else {
					// Usuário não tem vínculo com a empresa, então não pode solicitar nada
					$user_level = null;
				}

			} elseif ($ctpc == 3) {
				$user_level = 1; // Visitante em equipe privada
			}

		}		
	}
//COMPANY
}elseif(isset($_GET['company']) && !isset($_GET['profile']) && !isset($_GET['team']) && !isset($_GET['post']) && !isset($_GET['p'])){
	if(!empty($_GET['company'])){
		$js_page_type = 'company';		
		$company = search('cmp', 'companies', '', "id = '".$_GET['company']."'");
		$get_count = count($company);		
		if($get_count > 0){
			$company = $company[0];
			//COMPANY'S USERS
			$users = array();
			$users_level = array();					
			foreach(search('cmp', 'employees', 'us,st,nv', "st > 0 AND em = '".$_GET['company']."'") as $key => $data){			
				$users[] = $data['us'];				
				if ($data['nv'] > 0) {
					$users_level[] = $data['nv'];
				} else {							
					unset($users[$key]);
				}				
			}
				
			//COMPANY'S MODERATORS
			$moderators = array();			
			foreach(search('cmp', 'employees', 'us', "nv > 2 AND em = '".$_GET['company']."'") as $res){
				foreach($res as $key => $data){
					if($key == 'us'){
						$moderators[] = $data;
					}
				}		
			}
			//COMPANY'S REQUESTS
			$requests = array();			
			foreach(search('cmp', 'employees', 'us', "st = 0 AND em = '".$_GET['company']."'") as $res){
				foreach($res as $key => $data){
					if($key == 'us'){
						$requests[] = $data;
					}
				}		
			}			
			//COMPANY'S TEAMS		
			$teams = array_column(search('cmp', 'teams', 'id', "em = '".$_GET['company']."' AND pg <> 0"), 'id');
			
			//COMPANY'S POSTS			
			$company_posts = search('hnw', 'hpl', 'id', "em = '".$_GET['company']."'");
			//COMPANY'S DATA
			$pgft = 'em';
			$pgid = $company['id'];
			$pgpc = $company['pg'];
			$ctpc = $company['pc'];
			$pgtt = $company['tt'];
			$pgun = $company['un'];
			$pgim = $company['im'];
			$pgus = $company['us'];
			$pgtp = '';			
			$pgdt = $company['dt'];
			//NEW
			$pgbk = $company['bk'];
			$pgcl = $company['cl'];
			$pgds = $company['cf'];			
			//USER LEVEL IN COMPANY			
			$user_level = '';
			if(isset($_SESSION['wz'])){
				foreach(search('cmp', 'employees', 'us,st,nv', "em = '".$_GET['company']."' AND us = '".$_SESSION['wz']."'") as $reg){
					if ($reg['st'] == 0) {
						$user_level = 0; // Solicitação pendente
					} elseif ($reg['st'] == 1) {
						$user_level = $reg['nv']; // Nível efetivo
					}
				}
			} elseif($ctpc == 3) {
				$user_level = 1; // visitante tentando acessar página privada
			}
		}
	}
// FULL PAGE POST
}elseif (!isset($_GET['profile']) && !isset($_GET['team']) && !isset($_GET['company']) && (!empty($_GET['post']) || !empty($_GET['p']))) {
    
    $postId = $_GET['post'] ?? $_GET['p'];
    $postData = search('hnw', 'hpl', '', "id = '{$postId}'");

    if (!empty($postData)) {
        $post = $postData[0];
        $publisher = search('hnw', 'hus', '', "id = '{$post['us']}'")[0];
        $postUser = $publisher['tt'];
        $config = explode('|', $publisher['cf']);
        $pgpc = $post['pc'];

        $mba_id = $mba_us = $mba_st = [];

        if ($post['cm'] != 0) { // COMMUNITY POST
            foreach (search('cmp', 'teams_users', '', "cm = '{$post['cm']}'") as $member) {
                $mba_id[] = $member['id'];
                $mba_us[] = $member['us'];
                $mba_st[] = $member['st'];
            }
            $communityData = search('cmp', 'teams', '', "id = '{$post['cm']}'")[0];
            $config = explode('|', $communityData['cf']);

        } elseif ($post['em'] != 0) { // COMPANY POST
            foreach (search('cmp', 'employees', '', "em = '{$post['em']}'") as $employee) {
                $mba_id[] = $employee['id'];
                $mba_us[] = $employee['us'];
                $mba_nv[] = $employee['nv'];
                $mba_st[] = $employee['st'];
            }
            $companyData = search('cmp', 'companies', '', "id = '{$post['em']}'")[0];
            $config = explode('|', $companyData['cf']);
        }

        if (!empty($_GET['export'])) {
            header('Content-Type: application/octet-stream');
            header("Expires: 0");
            header("Cache-Control: must-revalidate, post-check=0, pre-check=0");
            header("Content-Disposition: attachment; filename=\"{$post['tt']}.doc\"");
            header('Content-Transfer-Encoding: binary');
            header('Pragma: public');
            ob_clean();
            flush();
        }
    }
}else{
	$pgpc = 1;
}
?>