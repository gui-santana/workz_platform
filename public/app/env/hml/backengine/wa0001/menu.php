...
<div id="divForm" class="large-12 medium-12 small-12 cm-pad-20 centered">
	...
	<div class="w-shadow w-rounded-15">
		...
		<div class="clearfix large-12 medium-12 small-12 text-ellipsis display-center-general-container text-ellipsis background-white border-t-input">
		...
		<div id="teamUsersContainer" class="large-12 medium-12 small-12 text-ellipsis background-white display-none"></div>
		<div class="large-12 medium-12 small-12 text-ellipsis display-center-general-container text-ellipsis background-white border-t-input">										
		...
		<div id="customFrequencyContainer" class="display-none"></div>
		<script>					
		(function() {
			'use strict';
			...
			function selectUsers(team){
				...
				teamUsersContainer.classList.remove('display-none');
				teamUsersContainer.classList.add('border-t-input');
				if(team > 0){
					let teamUsers = '<div class="large-12 medium-12 small-12 display-center-general-container"><div class="float-left large-2 medium-2 small-4 text-ellipsis cm-pad-15-l">Atribuído a</div>'+
									'<select id="teamUsers" class="large-10 medium-10 small-8 float-left border-none" style="height: 45px;" multiple></select></div>'+
					...
					teamUsersContainer.classList.add('display-none');
			...
			function toggleCustomFrequency(value) {
				...
				if (isCustom) {
					...
					customContainer.classList.add('display-none');
				}
			}
		...
		<div id="addButtonContainer" class="clearfix large-12 medium-12 small-12 text-ellipsis background-white border-t-input w-rounded-15-b" style="height: 45px">
...
?>
<div class="large-12 medium-12 small-12 text-center gray">
    <h2>Conexões WordPress</h2>
    <?php
    if(isset($_POST) && !empty($_POST)){
        $_POST = json_decode($_POST['vr'], true);
        
        if($_GET['action'] == 'wpcon_insert'){
            require_once $_SERVER['DOCUMENT_ROOT'] . '/functions/insert.php';
	        
    	    $us = $_POST['us'] ?? ""; //Nome de usuário
    	    $pw = $_POST['pw'] ?? ""; //Senha de aplicativo
    	    $ur = $_POST['ur'] ?? ""; //Caminho do site
    	    
    	    $en = base64_encode($us . ':' . $pw);
    	    
    	    if ($id = insert('app', 'wa0001_wp', 'us,en,ur', "'{$_SESSION['wz']}','{$en}','{$ur}'") ){
    	        echo '<small>Conexão registrada com sucesso.</small>';
    	    }
	    }elseif($_GET['action'] == 'wpcon_remove'){
	        require_once $_SERVER['DOCUMENT_ROOT'] . '/functions/delete.php';
	        
	        if($id = $_POST['id']){
	            if($wz = search('app', 'wa0001_wp', 'us', "id = '{$id}'")[0]['us']){
	                if($wz = $_SESSION['wz']){
	                    
	                    $pw = $_POST['pw_'.$id] ?? ""; //Senha de aplicativo
    	                $us = $_POST['us_'.$id] ?? ""; //Caminho do site
    	                $ur = $_POST['ur_'.$id] ?? ""; //Caminho do site
    	                
	                    $en = base64_encode($us . ':' . $pw);
	                    
	                    if($result = del('app', 'wa0001_wp', "id = '{$id}' AND us = '{$wz}' AND en = '{$en}' AND ur = '{$ur}'")){
	                        echo '<small>Conexão excluída com sucesso.</small>';
	                    }
	                }
	            }
	        }
    ...
    <div id="new" class="large-12 medium-12 small-12 cm-pad-20 centered">
        <div class="w-shadow w-rounded-15">
            <?php
            $i = 0;
            $wpcon = search('app', 'wa0001_wp', '', "us = '{$_SESSION['wz']}'");
            foreach($wpcon as $connection){
            $i++;
            $en = explode(':', base64_decode($connection['en']));
            $us = $en[0];
            $pw = $en[1];
            ?>
            
            <div class="large-12 medium-12 small-12 text-ellipsis display-center-general-container background-white position-relative border-t-input <?= ($i == 1) ? 'w-rounded-15-t' : '' ?>"
                 id="entry<?= $connection['id'] ?>"
                 data-id="<?= $connection['id'] ?>"
                 data-url="<?= $connection['ur'] ?>"
                 ondblclick="dispararConfirmacao(this)"
                 ontouchstart="startPress(this)" 
                 ontouchend="clearPress()">
                <div class="position-absolute abs-t-0 abs-r-0 abs-b-0 abs-l-0 z-index-1"></div>
                <input type="hidden" name="id" value="<?= $connection['id'] ?>"></input>
    			<input name="us_<?= $connection['id'] ?>" class="float-left border-none large-4 medium-4 small-4 cm-pad-5-l required" style="height: 45px" value="<?= $us ?>" placeholder="Nome de Usuário" disabled></input>
    			<input name="pw_<?= $connection['id'] ?>" class="float-left border-none large-4 medium-4 small-4 cm-pad-5-l required border-l-input" value="<?= $pw ?>" style="height: 45px" placeholder="Senha de Aplicativo" disabled></input>
    			<input name="ur_<?= $connection['id'] ?>" class="float-left border-none large-4 medium-4 small-4 cm-pad-5-l required border-l-input" value="<?= $connection['ur'] ?>" style="height: 45px" placeholder="Caminho do site" disabled></input>
    		</div>
            <?php
            }
            ?>
            <div id="divForm" class="large-12 medium-12 small-12 text-ellipsis display-center-general-container background-white border-t-input <?= ($i > 0) ? 'w-rounded-15-b' : 'w-rounded-15' ?>">
    			<input name="us" class="float-left border-none large-4 medium-4 small-4 cm-pad-5-l required" style="height: 45px" placeholder="Nome de Usuário"></input>
    			<input name="pw" class="float-left border-none large-4 medium-4 small-4 cm-pad-5-l required border-l-input" style="height: 45px" placeholder="Senha de Aplicativo"></input>
    			<input name="ur" class="float-left border-none large-4 medium-4 small-4 cm-pad-5-l required border-l-input" style="height: 45px" placeholder="Domínio do site"></input>
    		</div>
        </div>
        <div onclick="formValidator2('divForm', 'env/<?= $env ?>/backengine/wa0001/<?= $formAction ?>?action=wpcon_insert&qt=<?= $tipo ?>', 'config');" class="text-ellipsis text-left cm-pad-10 large-12 medium-12 small-12 w-color-bl-to-or pointer w-bkg-wh-to-gr w-shadow w-rounded-15 cm-mg-20-t">
    		<span class="fa-stack orange" style="vertical-align: middle;">
    			<i class="fas fa-circle fa-stack-2x light-gray"></i>
    	...
    	<small>Conexões para a Extensão do Workz! Tarefas para WordPress. (<a href="#">Saiba mais</a>)</small>
    </div>
    <script>
        (function(){
            'use strict';
            
             // Para mobile (long press)
            function startPress(element) {
                pressTimer = setTimeout(() => {
                    dispararConfirmacao(element);
            ...
            window.clearPress = clearPress;
            
            // Para desktop (duplo clique ou invocado pelo long press)
            function dispararConfirmacao(element) {
                const id = element.getAttribute('data-id');
                const url = element.getAttribute('data-url');
                sAlert(function() {
                    excluirConexao(id);
            	}, `Excluir conexão com ${url}?`, `Conexão excluída com sucesso.`, `A conexão não foi excluída`);
            }
            window.dispararConfirmacao = dispararConfirmacao;
        
            function excluirConexao(id) {
                formValidator2('entry' + id, 'env/<?= $env ?>/backengine/wa0001/<?= $formAction ?>?action=wpcon_remove&qt=<?= $tipo ?>', 'config');
            }
            window.excluirConexao = excluirConexao;
