...
if ((isset($_POST['vr']) && $_POST['vr'] !== '') || (isset($_GET['vr']) && $_GET['vr'] !== '')) {
	...
	if (count($vr) > 1) {
        ...
        // Verifica se $vr[0] é numérico para ações de tarefas específicas
        if (isset($vr[0]) && is_numeric($vr[0])) {
            $taskId = $vr[0];
            switch ($vr[2]) {
                case 'play': // Iniciar ou reiniciar tarefa
					...
					(function(){
						...
						goTo('core/backengine/wa0001/m_task.php', 'main-content', 1, <?= $taskId ?>);
					})();
					</script>
					<?php
                    break;

                case 'pause': // Pausar tarefa
                ...
                case 'stop': // Concluir tarefa					
					$holidays = array_column(search('app', 'wa0013_FerDiario', 'dt', ''), 'dt');					
                    $task = search('app', 'wa0001_wtk', 'time,init,pr,tml,step,st,uscm,lv,hb', "id = '$taskId'")[0] ?? null;
                    if($task){
						$st0 = $task['st'];
						$time = $task['time'];
                        $init = $task['init'];
						...
						$newEntry = [
							...
							"descrição" => base64_encode('(***!WORKZ!***)<i class="far fa-check-circle"></i> A tarefa com prazo em <strong>' . date('d/m/Y', strtotime($df)) . '</strong> foi concluída com sucesso por <a href="https://workz.com.br/?profile='.$_SESSION['wz'].'" target="_blank"><strong>'. $user .'</strong>.</a>') // Descrição codificada em Base64
						];
                ...
                case 'delete': // Excluir tarefa
                    ...
                    if ($previousData) {
                        del('app', 'wa0001_wtk', "id = '$taskId'");
                        del('app', 'wa0008_events', "el = '$taskId' AND ap = '1'");
			...
			switch ($action) {
				case 'step': // Atualizar passo de uma tarefa
					...
					if (update('app', 'wa0001_wtk', "step = '" . json_encode($steps, JSON_UNESCAPED_UNICODE) . "'", "id = {$taskId}")) {
						...
						echo "<script>goTo('core/backengine/wa0001/task_progress.php', 'task_progress', '0', '{$taskId}');</script>";						
					} else {
						echo json_encode(["error" => "Erro ao atualizar subtarefa"]);
					}
					break;

				case 'timeline': // Adicionar comentário na linha do tempo
					...
					if (update('app', 'wa0001_wtk', "tml = '" . json_encode($timeline, JSON_UNESCAPED_UNICODE) . "'", "id = {$taskId}")) {
						...
						echo "<script>goTo('core/backengine/wa0001/task_timeline.php', 'timeline', '0', '{$taskId}'); document.getElementById('textBox').innerHTML = '';</script>";
					} else {
