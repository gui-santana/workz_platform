...
if($_GET['qt'] == 3){
	$tskb = search('app', 'wa0001_wtk', '', "id = '".$_GET['vr']."'");
}else{
...
if((count($tskb) > 0) && ($tskd = search('app', 'wa0008_events', 'dt', "el = '".$tskr['id']."' AND ap = '1'"))){
	...
	<div id="folder-root" class="row large-10 medium-12 small-12 position-relative centered">
		...
		?>
		<div id="ckb_response" class=""></div>		
		
		<div class="large-12 medium-12 small-12 cm-pad-20 w-rounded-20 position-sticky z-index-2 w-shadow-1 background-white-transparent-75 backdrop-blur">
			<div class="large-12 medium-12 small-12 display-center-general-container">				
				<div class="float-left fs-g height-100 font-weight-500 text-ellipsis large-10 medium-8 small-6"><?= $tskr['tt'] ?></div>			
				<!-- MENU SUPERIOR DIREITO -->
				<div class="float-left large-2 mediun-4 small-6 text-right">
					<?php
					if($tskr['us'] == $_SESSION['wz']){
						
						//Se não estiver em execução
						if($tskr['st'] <> 2){
						?>					
						<span onclick="wAlert('core/backengine/wa0001/folder.php', 'folder-root', 1, '<?= $tskr['tg'].'|'.$tskr['id']; ?>', 'Deseja excluir permanentemente esta tarefa?', 'Tarefa excluída com sucesso.', 'A tarefa não foi excluída.');" class="fa-stack w-color-gr-to-gr pointer" style="vertical-align: middle;" title="Excluir permanentemente esta tarefa">
							<i class="fas fa-circle fa-stack-2x"></i>
							<i class="fas fa-trash fa-stack-1x fa-inverse"></i>
						</span>					
						<span onclick="editMode()" class="open-sidebar fa-stack w-color-gr-to-gr pointer" style="vertical-align: middle;" title="Editar tarefa">
							<i class="fas fa-circle fa-stack-2x"></i>
						...
						(function(){
							...
							function editMode(){
								toggleSidebar();							
								goTo('core/backengine/wa0001/menu.php', 'config', 1, '&id=<?= $tskr['id'] ?>');
							}
					...
					?>
					<span onclick="goTo('core/backengine/wa0001/folder.php', 'folder-root', 1, '<?= $tskr['tg'] ?>'); resetCounter();" class="fa-stack w-color-gr-to-gr pointer" style="vertical-align: middle;" title="Abrir a pasta da tarefa">
						<i class="fas fa-circle fa-stack-2x"></i>
						<i class="fas fa-folder fa-stack-1x fa-inverse"></i>
					</span>
					<span onclick="goTo('core/backengine/wa0001/main-content.php', 'main-content', '0', ''); resetCounter();" class="fa-stack w-color-bl-to-or pointer" style="vertical-align: middle;" title="Dashboard de tarefas">
						<i class="fas fa-circle fa-stack-2x"></i>
						<i class="fas fa-tasks fa-stack-1x fa-inverse"></i>
					</span>			
				</div>
			</div>			
			<div class="large-12 medium-12 small-12 z-index-2 cm-mg-15 cm-mg-0-h">
				<div class="float-left large-10 medium-10 small-8">
					<a><?= $dificuldade[$level] ?> (<?= $xpCalculado ?> Pts)</a>
				</div>
				<div class="float-left large-2 medium-2 small-4 height-100 text-right cm-pad-5-l text-ellipsis">
					<?php
					if($tskr['st'] == 2 && ($_GET['qt'] == 1 || $_GET['qt'] == 3)){
					?>
					<p class="float-right orange font-weight-600">
					...
					?>
					<p class="orange font-weight-600"><span><?= $formattedTime; ?></span></p>
					<?php										
					}
					?>		
					<div class="clear"></div>			
			...
			<div id="task_progress" class="large-12 medium-12 small-12 display-center-general-container"></div>										
		</div>
		<div class="large-12 medium-12 small-12 cm-mg-25-t cm-pad-15-t">					
			<?php				
			...
			<div class="large-12 medium-12 small-12">			
				...
				<div class="large-12 medium-12 small-12 cm-mg-20-b">
					<div class="large-3 medium-3 small-12 float-left cm-pad-20-r">
						<label class="font-weight-500">Criado por</label>
					</div>
					<div class="large-9 medium-9 small-12 float-right">
						<a target="_blank" class="w-color-bl-to-or pointer" href="https://workz.com.br/<?= $creator['un'] == '' ? '?profile='.$creator['id'] : $creator['un']; ?>"><?= $creator['tt']; ?></a>
					</div>
					<div class="clear"></div>
				</div>
				<?php
				...
				<div class="large-12 medium-12 small-12 cm-mg-20-b">
					<div class="large-3 medium-3 small-12 float-left cm-pad-20-r">
						<label class="font-weight-500">Equipe</label>
					</div>
					<div class="large-9 medium-9 small-12 float-right">						
						<a><?= search('cmp', 'teams', 'tt', "id = {$tskr['cm']}")[0]['tt'] ?></a>														
					</div>
					<div class="clear"></div>
				</div>
				<?php				
				...
				<div class="large-12 medium-12 small-12 cm-mg-20-b">
					<div class="large-3 medium-3 small-12 float-left cm-pad-20-r">
						<label class="font-weight-500">Atribuído a</label>
					</div>
					<div class="large-9 medium-9 small-12 float-right">
						<?php
						foreach($uscm as $user){
						?>
						<a><?= ($user == $_SESSION['wz']) ? 'mim' : search('hnw', 'hus', '', "id = '".$user."'")[0]['tt'] ?><?= ($n == $totalUsers) ? '' : (($n == ($totalUsers - 1)) ? ' e a ' : ', a ') ?></a>
						<?
						$n++;
						}
						?>									
				...
				<div class="large-12 medium-12 small-12 cm-mg-20-b">
					<div class="large-3 medium-3 small-12 float-left cm-pad-20-r">
						<label class="font-weight-500">Observação</label>
					</div>
					<div class="large-9 medium-9 small-12 float-right">								
						<?= nl2br($tskr['ds']) ?>								
					</div>
				...
				<div class="large-12 medium-12 small-12 cm-mg-20-b">
					<div class="large-3 medium-3 small-12 float-left cm-pad-20-r">
						<label class="font-weight-500">Recorrência</label>
					</div>																				
					<div class="large-9 medium-9 small-12 float-right">
						<?= $recurrenceOptions[$tskr['pr']] ?>
					</div>
					<div class="clear"></div>
				</div>
				<?php 
				...
				<div class="large-12 medium-12 small-12 cm-mg-20-b">
					<div class="large-3 medium-3 small-12 float-left cm-pad-20-r">
						<label class="font-weight-500">Prazo fatal</label>
					</div>
					<div class="large-9 medium-9 small-12 float-right">
						<?= ($df == 0) ? 'Indefinido' : ucfirst(strftime('%A, %d de %B de %Y, às %H:%M', strtotime($df))) ?>									
					</div>
					<div id="tms_response" class="clear"></div>
				</div>				
				<?php 
				//ETAPAS 
				?>						
					...
					<div class="large-9 medium-9 small-12 float-right border-like-input cm-pad-20 w-rounded-15 background-white-transparent-50">
						...
						foreach($steps as $key => $step){											
							...
							<div class="large-12 medium-12 small-15 cm-pad-5 cm-pad-0-l">
								<input onchange="updateStepStatus(<?= $key ?>, this.checked)" id="stepckb<?= $key; ?>" name="step" type="checkbox" <?= ($step_stat == 1) ? 'checked' : '' ?> <?= $isDisabled ?> value="<?= $key ?>"></input>
								<label for="stepckb<?= $key ?>"><?= (!empty($step_date)) ? ' <strong class="orange">'.date('d/m/Y H:i', strtotime($step_date)).' | </strong>' : '' ?> <?= str_replace('\'', '', $step_desc) ?></label>
							</div>								
							<?php										
						}									
						?>
				...
				(function () {
					...
					function handleTaskAction(tskr, action, question, successMsg, failMsg) {
						sAlert(function() {
							switch (action) {
								case 'stop':
									// Concluir tarefa
									goTo('core/backengine/wa0001/process.php', 'ckb_response', '0', tskr.id + '|' + tskr.st + '|stop|');
									resetCounter();										
									break;
									
								case 'pause':
									// Pausar tarefa
									goTo('core/backengine/wa0001/process.php', 'ckb_response', '0', tskr.id + '|' + tskr.st + '|pause|');
									resetCounter();										
									break;

								case 'play':
									// Iniciar ou Re-iniciar tarefa												
									goTo('core/backengine/wa0001/process.php', 'ckb_response', '0', tskr.id + '|' + tskr.st + '|play');																						
									break;

								case 'eject':
									// Arquivar tarefa
									goTo('core/backengine/wa0001/process.php', 'ckb_response', '', tskr.id + '|' + tskr.st + '|eject');										
									break;
					...
					goTo('core/backengine/wa0001/task_progress.php', 'task_progress', '0', '<?= $tskr['id'] ?>');
					goTo('core/backengine/wa0001/task_timeline.php', 'task_timeline', '0', '<?= $tskr['id'] ?>');					
				})();
				</script>						
			</div>									
		</div>
		...
		//EDITOR DE TEXTO
		?>
		<div class="large-12 medium-12 small-12 centered background-white w-shadow-1 w-rounded-20 cm-pad-25-h cm-pad-25-t cm-mg-25-t">					
			<div>
				...
				<div id="editContainer" class="large-12 medium-12 small-12 overflow-auto text-right background-white cm-mg-10-t">
					<div id="textBox" contenteditable="true" class="required float-left large-12 medium-12 small-12 text-left cm-pad-20 border-like-input w-rounded-20 position-relative"></div>
					<span onclick="sendToTimeline()" style="top: -40px;" class="position-absolute abs-r-5 fa-stack w-color-wh-to-gr pointer fs-f" title="Enviar">
						<i class="fas fa-circle fa-stack-2x"></i>
				...
				<div id="tml_response"></div>
			</div>			
											
		</div>				
		<div class="large-12 medium-12 small-12 float-left cm-pad-20-t">			
			<div id="tml-test">								
			</div>
		</div>
		<?php
		...
		?>		
		<div id="task_timeline" class="large-12 medium-12 small-12 centered cm-mg-25-t w-task-tl-timeline"></div>
	</div>
	<?php			
}else{
