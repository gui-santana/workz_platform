...
if (isset($_GET) && count($_GET) > 0) {
	...
	if ((isset($_GET['qt']) && $_GET['qt'] === '0') || isset($_GET['w'])) {
		...
		?>
		<div class="row large-10 medium-10 small-12 cm-pad-30 position-relative centered">
			<div class="large-12 medium-12 small-12 display-center-general-container cm-mg-15-t">				
				<div class="float-left fs-g height-100 font-weight-500 large-12 medium-12 small-12 orange display-center-general-container">
					<a class="text-ellipsis">Minhas páginas</a>
				</div>				
			</div>
			<div class="cm-mg-15 cm-mg-0-h large-12 medium-12 small-12 w-rounded-15 z-index-2 w-shadow-1 background-white-transparent-75 backdrop-blur">
				<div class="large-12 medium-12 small-12 fs-c position-relative overflow-x-auto">
					<div style="min-width: 800px" class="clearfix cm-pad-30-h cm-pad-10 cm-pad-15-t large-12 medium-12 small-12 position-relative text-ellipsis border-b-input overflow">	
						<div class="float-left large-4 medium-4 small-4 text-ellipsis font-weight-500">Nome</div>						
						<div class="float-left large-3 medium-3 small-3 text-ellipsis font-weight-500 text-center">Modificado em</div>		
						<div class="float-left large-3 medium-3 small-3 text-ellipsis font-weight-500 text-center">Tipo</div>
						<div class="float-left large-2 medium-2 small-2 text-ellipsis font-weight-500 text-center">Data de publicação</div>						
					</div>		
					<?php
					$tipos = [
						1 => "Documento",
					...
					<div style="min-width: 800px" class="clearfix cm-pad-10 cm-pad-30-h large-12 medium-12 small-12 position-relative w-color-bl-to-or w-bkg-tr-gray <?= ($i == $totalResults) ? 'w-rounded-15-b'  : 'border-b-input' ?>">
						<label title="<?= $myPostsResult['tt']; ?> - Clique para ver" onclick="<?php if($myPostsResult['tp'] == 1){ ?> goPost('env/<?= $env ?>/backengine/wa0006/tab_content.php', 'tab', <?= $myPostsResult['id'] ?>, '') <?php }else{ ?> goPost('env/<?= $env ?>/backengine/wa0006/tab_content.php', 'tab', '<?= $myPostsResult['id']; ?>', '') <?php } ?>" class="pointer">
							<div class="float-left large-4 medium-4 small-4 text-ellipsis-2 font-weight-400 cm-pad-20-r break-word"><?= $myPostsResult['tt']; ?><?= ($myPostsResult['us'] !== $_SESSION['wz']) ? '<i class="fas fa-people-arrows cm-mg-10-l"></i>' : '' ?></div>
							<div class="float-left large-3 medium-3 small-3 text-ellipsis text-center"><?= (!empty($myPostsResult['dc'])) ? date('d/m/Y', strtotime($myPostsResult['dc'])) : 'Data indefinida' ?></div>
							<div class="float-left large-3 medium-3 small-3 text-ellipsis text-center"><?= $tipos[$myPostsResult['tp']] ?></div>				
							<div class="float-left large-2 medium-2 small-2 text-ellipsis text-center"><?= ($myPostsResult['dt'] != 0 && $myPostsResult['st'] > 0) ? date('d/m/Y', strtotime($myPostsResult['dt'])) : 'Não publicado' ?></div>						
						</label>				
					</div>
					<?php
					$i++;					
 ...
 */
	...
	?>		
	<div id="result" class="display-none"></div>
	<script>
	(function(){
		...
		function appHeader() {
			const headerHTML = 	'<div class="height-100 display-center-general-container large-12 medium-12 small-12">'+
									...
									'<div class="clearfix large-6 medium-6 small-6">'+
		...
		function delPage(){
			sAlert(function(){
				closeInstance();
				goTo('env/<?= $env ?>/backengine/wa0006/tab_content.php', 'tab', '0', 'del|<?= $id;?>');
		...
		function editPage(id,tp){			
			goTo('env/<?= $env ?>/backengine/wa0006/config.php', 'config', tp, id);			
		}
		...
		function publishPage() {			
			...
			sAlert(function() {				
				goPost('env/<?= $env ?>/backengine/wa0006/save.php', 'result', vr, '');							
	...
	if($tp == 1){
		//SUNEDITOR
		?>			
		<link href="env/<?= $env ?>/backengine/wa0006/suneditor/dist/css/suneditor.min.css" rel="stylesheet">		
		<link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/codemirror@5.49.0/lib/codemirror.min.css">		
		<link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/katex@0.11.1/dist/katex.min.css">
		<link rel="stylesheet" href="app/env/<?= $env ?>/backengine/wa0006/suneditor/dist/css/se_viewer.css" />
		...
		(function() {
			...
			function mainMenu(){
				closeInstance(); 
				goTo('env/<?= $env ?>/backengine/wa0006/tab_content.php', 'tab', '0', '');
				customHeader.innerHTML = '';
				customHeader.classList.add("display-none");
			}
			...
			function loadEditor(id, st){
				...
				function saveContent(id){
					...
					fetch('env/<?= $env ?>/backengine/wa0006/save.php', {
						method: 'POST',
						headers: {
	...
	// Parâmetros para páginas do tipo "Página da Web"
		...
		<div id="gjs-response" class="display-none"></div>
