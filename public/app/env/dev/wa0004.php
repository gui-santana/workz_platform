...
if(count($user_pass) == 0){
	...
	if($user_pass[0]['s1'] == 'true'){
		if(!isset($_POST['authCode'])){
			...
			?>
			<div style="height: calc(100% - 106.14px)" class="large-12 medium-12 small-12 overflow-y-auto display-center-general-container">
				
				<div class="clearfix large-10 medium-10 small-10 cm-mg-30-t position-relative centered fs-e cm-pad-15 background-white w-rounded-30 w-shadow-1 cm-mg-20-t cm-pad-30" style="color: <?= $colors[0] ?>">				
					<form method="post" action="">			
					<div class="large-12 medium-12 small-12 cm-mg-30-b text-center">
						<h3 class="cm-mg-15-b">Autenticação em 2 fatores</h3>
						<label><p><?= $msg ?></p><br/><p>Por favor, insira o código aqui para entrar:</p></label>						
						<div class="large-6 medium-6 small-12 centered cm-mg-15-t">
					...
					<div class="large-6 medium-6 small-12 centered">						
						<button type="submit" style="color: <?= $colors[0] ?>;" class="font-weight-500 border-none w-bkg-tr-gray display-center-general-container cm-pad-10 large-12 medium-12 small-12 text-center w-rounded-30 w-shadow-1 pointer fs-c">
	...
	update('app', 'wa0004_keys', "ty = ''", "us = '".$_SESSION['wz']."'");
	?>	
	<div style="height: calc(100% - 217.72px)" class="content container large-12 medium-12 small-12 overflow-auto cm-pad-20-b cm-pad-20 ease-all-2s clearfix">
		<div name="opt1" class="view ease-all-2s clear">
			<div class="tab large-10 medium-12 small-12 centered cm-pad-15 cm-pad-10-h">
				<input class="w-rounded-30 w-shadow-1 border-like-input input-border cm-pad-15 large-12 medium-12 small-12" type="text" placeholder="Pesquisar" onkeyup=""/>		
			</div>	
			<div class="large-10 medium-12 small-12 centered clearfix" id="tabelaSenhas"></div>
		</div>			
		<div name="opt2" class="cm-pad-10 cm-pad-30-b display-none ease-all-2s opacity-0">
			<div class="display-center-general-container cm-pad-10 cm-pad-20-b">
				<div style="height: 70px; width: 70px; color: <?=  $colors[0] ?>; " class="text-center position-relative display-center-general-container fs-f float-left display-block">
				...
				<h3 class="display-block cm-mg-15-l" style="color: <?=  $colors[0] ?>;">Adicionar</h3>
			</div>			
			<div class="cm-mg-30-t position-relative centered large-10 medium-12 small-12 fs-e cm-pad-15 background-white w-rounded-30 w-shadow-1 cm-mg-20-t cm-pad-30" style="color: <?=  $colors[0] ?>">
				...
				<form id="formSenha " method="POST" class="cm-mg-25-t">
					...
					<div class="large-6 medium-6 small-12 float-right">						
						<div onclick="submitForm(this)" style="color: <?= $colors[0] ?>;" class="w-bkg-tr-gray display-center-general-container cm-pad-10 large-12 medium-12 small-12 text-center w-rounded-30 w-shadow-1 pointer fs-c">
								<div style="height: 30px; width: 30px; color: <?= $colors[0] ?>;" class="text-center display-center-general-container fs-b float-left display-block white">		
		...
		<div name="opt3" class="cm-pad-10 cm-pad-30-b display-none ease-all-2s opacity-0">		
			<div class="display-center-general-container cm-pad-10 cm-pad-30-b">
				...
				<h3 class="display-block cm-mg-15-l" style="color: <?= $colors[0] ?>;">Gerar Senha</h3>
			</div>
			<div class="cm-mg-30-t position-relative centered large-10 medium-12 small-12 fs-e cm-pad-15 background-white w-rounded-30 w-shadow-1 cm-mg-20-t cm-pad-30" style="color: <?= $colors[0] ?>">
				<div style="height: 40px; width: 40px; top: -15px" class="text-center position-absolute abs-l-30 display-center-general-container fs-f display-block white"><div class="position-absolute abs-t-0 abs-l-0 w-rounded-10 height-100 large-12 medium-12 small-12 background-white w-shadow-2 z-index-0" style="transform: rotate(20deg); background: <?= $colors[0] ?>;"></div><i class="fas fa-key pointer z-index-1 centered"></i></div>
				...
				<div class="large-6 medium-6 small-12 float-right">						
					<div onclick="gerarSenhaAleatoria(document.getElementById('password_length').value)" style="color: <?= $colors[0] ?>;" class="w-bkg-tr-gray display-center-general-container cm-pad-10 large-12 medium-12 small-12 text-center w-rounded-30 w-shadow-1 pointer fs-c">
						<div style="height: 30px; width: 30px; color: <?= $colors[0] ?>;" class="text-center display-center-general-container fs-b float-left display-block white">
							<div class="position-absolute abs-t-0 abs-l-0 w-rounded-10 height-100 large-12 medium-12 small-12 background-white z-index-0 w-shadow" style="transform: rotate(20deg);"></div>
	...
	<div style="" class="abs-b-0 position-fixed cm-pad-20-h cm-pad-30-b large-12 medium-12 small-12 z-index-2">
		<div class="cm-pad-5 large-10 medium-12 small-12 w-modal-shadow text-center fs-f background-white display-center-general-container centered overflow-x-auto w-rounded-20-35">
			<div class="centered">			
				<div class="float-left text-center cm-pad-10 cm-pad-15-h line-height-a w-color-gr-to-gr pointer" id="opt1" onclick="option(this)">
					<i class="fas fa-lock fs-g pointer cm-pad-5-t"></i><br/>
					<a class="fs-a">Senhas</a>
				</div>
				<div class="float-left text-center cm-pad-10 cm-pad-15-h line-height-a w-color-gr-to-gr pointer"  id="opt2" onclick="option(this)">
					<i class="fas fa-plus-circle fs-g pointer cm-pad-5-t"></i><br/>
					<a class="fs-a">Adicionar</a>
				</div>
				<div class="float-left text-center cm-pad-10 cm-pad-15-h line-height-a w-color-gr-to-gr pointer"  id="opt3" onclick="option(this)">
					<i class="fas fa-magic fs-g pointer cm-pad-5-t"></i><br/>
	...
	<?php
	include('backengine/wa0001/user_cmp.php');
	?>
	<script>
		function appOptions() {
			const optionsHTML = '<div class="w-shadow background-white w-rounded-15">' +
					...
					createOption("env/<?= $env ?>/backengine/wa0001/menu.php", "config", 1, "Nova Tarefa", "fa-plus", true, false, true) +
				'</div>';        
		...
		$(document).on('change', '.onoffswitch-checkbox', function () {
			...
			if(el.split('-')[0] == 'option'){		
				$.ajax({
				url: 'https://workz.com.br/app/env/<?= $env ?>/backengine/wa0004/senhas.php',							
				type: 'POST',
				dataType: 'json',
				data: {
					fnc: 'options',
		...
		function carregarSenhas(){
			goTo('env/<?= $env ?>/backengine/wa0004/list.php', 'tabelaSenhas', '', '');
		}
		...
		function saveChanges(el){
			...
			data: {
				...
				id: parent.id,
				team: document.getElementById(parent.id + '_team').value,
				wz: <?= $_SESSION['wz'] ?>
			},
			success: function(response){
			...
			if (form[0].checkValidity()) {
				...
				$.ajax({
					...
					data: {
						...
						senha: senha,
						wz: <?= $_SESSION['wz'] ?>,
						<?= session_name() ?>: '<?= session_id() ?>' // Adicione esta linha
					},
					success: function (response) {
						form.trigger('reset');
						carregarSenhas();
					...
					'<div class="w-shadow background-white w-rounded-15">'+
						'<div class="large-12 medium-12 small-12 cm-pad-10 cm-pad-5-l display-flex">'+
							...
							'<div class="onoffswitch align-right">'+
								'<input type="checkbox" name="onoffswitch" class="onoffswitch-checkbox" id="option-1" tabindex="0" <?= ($user_pass[0]['s1'] == 'true') ? 'checked' : '' ?>>'+
						...
						'<div class="large-12 medium-12 small-12 cm-pad-10 cm-pad-5-l border-t-input display-flex ">'+
							...
							'<div class="onoffswitch align-right">'+
								'<input type="checkbox" name="onoffswitch" class="onoffswitch-checkbox" id="option-2" tabindex="0" <?= ($user_pass[0]['s1'] == 'true') ? 'checked' : '' ?>>'+
