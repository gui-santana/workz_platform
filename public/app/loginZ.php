			...
			if (!filter_var($ml, FILTER_VALIDATE_EMAIL)) {
				...
				if ($hus && is_array($hus) && !$renew) {
					if (password_verify($pw, $hus[0]['pw'])) {                    						
						...
						?><script>goPost('https://app.workz.com.br/workauth.php', 'login', <?= $session_data ?>, '');</script><div id="login-success"></div><?php
						exit();
					} else {
...
<div id="loginZ" class="form-group">
	...
	if(isset($newMl)){
		...
		<div class="large-12 medium-12 small-12 text-ellipsis fs-e cm-pad-15-b">
			<div onclick="goTo('https://workz.com.br/partes/loginZ.php', 'login', '&bk=init', '');" class="display-center-general-container w-color-wh-to-or pointer">				
	...
	<div class="large-12 medium-12 small-12 clearfix display-center-general-container">
		<!-- GOOGLE OAUTH -->
		<div class="google-login-wrapper display-none">
			<div id="g_id_onload" data-client_id="427324182547-j9bubtdhacjqe8i0592n2e4jd6b6unm2.apps.googleusercontent.com" data-login_uri="https://workz.com.br/includes/Google/oauth_callback.php" data-callback="onGoogleSignIn" data-auto_prompt="false"></div>			
		</div>
		<div class="g_id_signin cm-mg-0-l" data-type="icon" data-size="large" data-text="signin_with" data-shape="pill" data-logo_alignment="left" style="margin-top: -21.61px; width: 50px;"></div>
		<!-- BOTÃO ENTRAR -->
		<div style="height: 40px; width: calc(100% - 50px);" onclick="loginSubmit()" type="submit" class="w-rounded-30 w-shadow-1 font-weight-500 pointer w-all-or-to-bl border-none fs-e input-shadow display-center-general-container">
...
<script>
	function onGoogleSignIn(response) {
		// Captura o token retornado pelo Google
		const credential = response.credential;

		// Prepara os dados para enviar via AJAX
		const data = {
			credential: credential, // Token do Google
			geolocation: document.getElementById('geolocation').value,
			source: 'google_login' // Identificador opcional
		};

		// Envia os dados para oauth_callback.php
		goPost('https://workz.com.br/includes/Google/oauth_callback.php', 'login', data, '');
	}
	...
	function codeCheck(){
		...
		if(codeField.value !== '' && mailField.value !== ''){
			goTo('https://workz.com.br/partes/loginZ.php', 'login', '&cf=' + codeField.value + '&ml=' + mailField.value, '');
		}else{
	...
	function isStrongPassword(password) {
	  ...
	  function loginSubmit() {
		...
		if (mailField.value === '' && passwordField.value === '') {			
			goPost('https://workz.com.br/partes/loginZ.php', 'login', '', '');
			return;
		}		
		...
		formValidator2('loginZ', 'https://workz.com.br/partes/loginZ.php', 'login');
	}

	
	function resetPassword(){
		const mailField = document.getElementById("user_mail");		
		goTo('https://workz.com.br/partes/loginZ.php', 'login', '', mailField.value);	
	}
